...
class Trainer(object):
    ...
    def _set_env_vars(self):
        ...
        del env
        self.envs = [functools.partial(self.make_env, i) for i in range(self.num_env)]
    ...
    def train(self, args):
        ...
        checkdir = osp.join(logger.get_dir(), 'checkpoints', args['env']+'-'+args['policy'])
        ...
        while True:
            ...
            if info['update']:
                print('task = ', args['env'], ' num_env = ', args['num_env'], ' policy = ', args['policy'])
                info['update']['n_updates'] += start_nupdates
                info['update']['tcount'] += start_nupdates*args['nsteps_per_env']*args['num_env']
...
def get_experiment_environment(**args):
    ...
    logger_context = logger.scoped_configure(dir=args['env']+'-'+args['policy'],
...
def add_environments_params(parser):
    ...
    parser.add_argument('--env', help='environment ID', default= 'Maze1203Env-v3',
                        type=str)
...
def add_rollout_params(parser):
    parser.add_argument('--nsteps_per_env', type=int, default=2000)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--policy', type=str, default='idf',
                        choices=["none", "idf", "rnd", "vaesph", "vaenonsph", "pix2pix"])
    parser.add_argument('--num_vf', type=int, default=1, choices=[1,2])

    args = parser.parse_args()

    buf_size = args.num_env * args.nsteps_per_env
    while buf_size > 2.0**16.0:
        args.nminibatches *=2
        buf_size /= 2
